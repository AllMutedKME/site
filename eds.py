import random
def eds(private_key = 1023456789012345, message = 123456):
    ...
    # test starts here
      # any random integer ДЛЯ ВВОДА ПОЛЬЗОВАТЕЛЕМ
    public_key = g_point.multiply(private_key)
      # any integer    ТАКЖЕ ДЛЯ ВВОДА ПОЛЬЗОВАТЕЛЕМ

    signature = sign_message(message, private_key)

    verif = verify_signature(signature, message, public_key)
    return signature
